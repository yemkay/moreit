...
class Arc90_Service_Twitter_Response {
     ...
     */
    public function __construct($data, TwitterOAuth $to) {
        $this->_data = $data;
        $this->_metadata = $to->http_info;
        //$this->_headers = $headers;

        if (!defined('TWITTER_ERROR_LIMIT_EXCEEDED'))
            define('TWITTER_ERROR_LIMIT_EXCEEDED', 400);
        if (!defined('TWITTER_ERROR_NOT_FOUND'))
            define('TWITTER_ERROR_NOT_FOUND', 404);
        if (!defined('TWITTER_ERROR_FORBIDDEN'))
            define('TWITTER_ERROR_FORBIDDEN', 403);
        if (!defined('TWITTER_DOWN'))
            define('TWITTER_DOWN', 502);
    }
    ...
    /**
     * Returns the JSON object from the content body (if any) returned by Twitter.
     *
     * @return string
     */
    public function getJsonData() {
        return json_decode(utf8_encode($this->_data));
    }
    ...
    public function getError() {
        ...
        if (is_array($errors))
            foreach ($errors as $e)
                $error[] = sprintf('Code: %s, Message: %s', $e->code, $e->message);
    ...
    public function getHttpCode() {
        switch ($this->_metadata['http_code']) {
            case 400:
                return TWITTER_ERROR_LIMIT_EXCEEDED;
                break;
            case 404:
                return TWITTER_ERROR_NOT_FOUND;
                break;
            default:
                return $this->_metadata['http_code'];
        }
    }

    /* When an application exceeds the rate limit for a given API endpoint, the Twitter API will now return an
      HTTP 429 “Too Many Requests” response code instead of the variety of codes you would find across
      the v1's Search and REST APIs.
      {
      "errors": [
      {
      "code": 88,
