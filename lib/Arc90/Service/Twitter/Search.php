...
class Arc90_Service_Twitter_Search
{
    ...
    const PATH_SEARCH = '/search/tweets';
     ...
     * @return Arc90_Service_Twitter_Response
     */
    protected function _sendRequest($uri, $params)
    {
        if (!isset($this->user_secret))
            self::_throwException('OAUTH details missing');
        
        require_once('oauth/twitterOAuth.php');

        /* Create TwitterOAuth with app key/secret and user access key/secret */
        $to = new TwitterOAuth($this->oauth_consumer_key, $this->oauth_consumer_secret, 
                $this->user_token, $this->user_secret);

        log_message('debug', 'Search request: '. print_r($uri, true));
        
        $uri = parse_url($uri, PHP_URL_PATH);
        
        $content = $to->get( self::API_URI.$uri, $params);
        
        //log_message('debug', 'Search response: '. print_r($content, true));

        return new Arc90_Service_Twitter_Response(json_encode($content), $to);
    }
