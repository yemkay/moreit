        ...
        if (!empty($m[2])) {
            ...
            switch ($type) {
                case 'a':
                    ...
                    return join(', ', $parts);
                case 'm': //to handle multiple inserts in a single query. ?# contains the keys and ?m is an array of records (array of values)
                    if (!$value) $this->_placeholderNoValueFound = true;
                    if (!is_array($value)) return 'DBSIMPLE_ERROR_VALUE_NOT_ARRAY';
					...
					foreach ($value as $record_key=>$record_value) {
						...
						foreach ($record_value as $k=>$v) {
							$parts[] = $v === null? 'NULL' : $this->escape($v);
						}
